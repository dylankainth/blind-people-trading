...
raw_df = yf.Ticker('SOL-USD').history(period='730d', interval='1h').reset_index()
...
df_scaled[config['all_features']] = feature_scaler.transform(processed_df[config['all_features']])
df_scaled['Original_Log_Close'] = processed_df['Log_Close'].copy()

# Generate prediction
last_seq = torch.FloatTensor(df_scaled.iloc[-config['T']:][config['all_features']].values).unsqueeze(0)
with torch.no_grad():
